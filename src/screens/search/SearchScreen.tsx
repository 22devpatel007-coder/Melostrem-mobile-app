...
// afeter devlopment replace with mmkv in all file player homescreen and search screen
import AsyncStorage from '@react-native-async-storage/async-storage';
// TODO: Replace AsyncStorage with MMKV after dev build setup
const HISTORY_KEY = 'search_history';
const MAX_HISTORY = 8;

function readHistory(): string[] { return []; }
function pushHistory(query: string): void {
  AsyncStorage.getItem(HISTORY_KEY).then(raw => {
    const prev = raw ? JSON.parse(raw) : [];
    AsyncStorage.setItem(HISTORY_KEY, JSON.stringify([query, ...prev.filter((q:string) => q !== query)].slice(0, MAX_HISTORY)));
  });
}
function removeEntry(query: string): string[] {
  AsyncStorage.getItem(HISTORY_KEY).then(raw => {
    const prev = raw ? JSON.parse(raw) : [];
    AsyncStorage.setItem(HISTORY_KEY, JSON.stringify(prev.filter((q:string) => q !== query)));
  });
  return [];
}
function clearHistory(): void { AsyncStorage.setItem(HISTORY_KEY, '[]'); }
